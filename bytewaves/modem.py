...
def text_to_sound(text):
    ...
    # Add CRC32 checksum for data integrity
    crc = zlib.crc32(data_bytes)
    data_with_crc = data_bytes + crc.to_bytes(4, 'big')

    # Determine packet size n (data + crc + ecc)
    n = len(data_with_crc) + N_ECC_BYTES

    if n > MAX_BLOCK_SIZE:
        max_data_len = MAX_BLOCK_SIZE - N_ECC_BYTES - 4  # 4 bytes for CRC
        print(f"Error: Data is too long ({len(data_bytes)} bytes). Maximum is {max_data_len} bytes.")
        return np.array([])

    try:
        # Use a dynamic RSCodec for variable-length packets
        rs_codec = RSCodec(N_ECC_BYTES, nsize=n)
        encoded_message = rs_codec.encode(data_with_crc)
        print(f"Original data: {len(data_bytes)} bytes. With CRC+ECC: {len(encoded_message)} bytes.")
    except Exception as e:
        print(f"Error during Reed-Solomon encoding: {e}")
        return np.array([])

    # Construct the full packet: Preamble + Length (n) + Encoded Data
    length_byte = bytes([n])
...
def attempt_decode(accumulated_scores, n, transmissions_count):
    ...
    try:
        # Initialize the codec with the correct packet size n
        rs_codec = RSCodec(N_ECC_BYTES, nsize=n)
        decoded_message, decoded_ecc = rs_codec.decode(best_guess_bytes)
        ...
        if received_crc != expected_crc:
            ...
            print(f"Expected CRC: {expected_crc}, Received CRC: {received_crc}")
            return False
...
def receive_loop():
    ...
    expected_packet_size_n, byte_idx, transmissions_count = 0, 0, 0
    accumulated_scores = []
        ...
        while not stop_receiving_flag.is_set():
            try:
                ...
                if state == "HUNTING":
                    ...
                    # Check for a valid length (must be big enough for CRC and ECC)
                    if N_ECC_BYTES + 4 < expected_packet_size_n <= MAX_BLOCK_SIZE:
                        print(f"Packet length received: {expected_packet_size_n} bytes. Starting collection.")
